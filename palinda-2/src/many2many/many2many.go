...
func main() {
	...
	//close(ch)  //if close before wait program panics trying to send to closed chanel
...
func Produce(id string, n int, ch chan<- string, wg *sync.WaitGroup) {
	...
	wg.Done()
	//close(ch) // sender closes the chanel causing it to try and send on a closed chanel => panic
}

// Consume prints strings received from the channel until the channel is closed.
func Consume(id string, ch <-chan string) {
	for s := range ch {
		RandomSleep(200) // Simulate time to consume data.
